...
| `_app_users`           | new API only | new API                   | `_id`, `email`, `username`, `passwordHash`, timestamps | unique `email`, unique `username`         | read/write              |
| `_app_refresh_tokens`  | new API only | new API                   | `_id`, `jti` (hash), `userId`, `expiresAt`             | unique `jti`; optional TTL on `expiresAt` | read/write              |
...
- **Index policy:**
  ...
  - **App-owned** `_foundation_*` and `_app_*` collections: the new app may manage indexes **after** the inventory row is approved (schemas land in T14–T15).
- Planned indexes for auth collections (enforce in Mongoose schemas later):
  - `_app_users`: **unique** on `email`; **unique** on `username`
  - `_app_refresh_tokens`: **unique** on `jti` (store hash of JWT id, not raw token); optional **TTL** index on `expiresAt`
...
## `_app_users` (platform auth — ADR-003)

App-owned password accounts for the new API. **Not** the legacy `users` collection.
...
Writers: Auth / Users module in `apps/api` (T15+). Legacy must not write here. Dual-read/link with legacy `users` is **ADR-004** (out of platform scope).

## `_app_refresh_tokens` (platform auth)
...
| `userId`    | ObjectId | Ref to `_app_users._id`                          |
...
Authoritative writer: **legacy app**. New app mode: **read-only until ADR-004**. No inserts/updates/deletes from `apps/api` until ADR-004 + inventory mode change + feature flag. Consistent with [ADR-001](../adr/001-shared-mongodb-with-legacy.md).
