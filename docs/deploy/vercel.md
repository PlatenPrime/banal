...
**Staging and production are separate Vercel projects** with distinct `VITE_API_URL` values pointing at the matching Railway API. Production custom domain: `app.banal.app` ([Custom domain](#custom-domain-appbanalapp), [cookie-cutover.md](cookie-cutover.md)).
...
| -------------- | ----------------------------------------- | ------------------------------------------------------------------------------- | -------- | ------------------------------------------ |
| `VITE_API_URL` | `https://api-staging-9c27.up.railway.app` | `https://api.banal.app` (interim: `https://api-production-b6c9.up.railway.app`) | yes      | Absolute API origin; embedded in client JS |
...
| Variable                   | Staging API                            | Production API (after T23 cutover)               |
| -------------------------- | -------------------------------------- | ------------------------------------------------ |
| `WEB_ORIGIN`               | `https://banal-web-staging.vercel.app` | `https://app.banal.app`                          |
| `WEB_ORIGIN_PREVIEW_REGEX` | `^https://.*\.vercel\.app$`            | unset                                            |
| `COOKIE_DOMAIN`            | unset                                  | `.banal.app`                                     |
| `AUTH_COOKIE_SAMESITE`     | `none`                                 | `lax` (interim `none` until custom domains live) |

## Custom domain (`app.banal.app`)

**Gate:** parent `banal.app` must be registered with editable DNS. Until then keep the `*.vercel.app` production URL.

1. Vercel → project `banal-web-production` → **Settings** → **Domains** → Add `app.banal.app`.
2. Copy the DNS record Vercel shows (usually **CNAME** `app` → `cname.vercel-dns.com` or the project-specific target).
3. Create the record at the DNS provider for `banal.app`.
4. Wait until Domains UI shows **Valid** and the browser padlock works on `https://app.banal.app`.
5. Smoke: `WEB_BASE_URL=https://app.banal.app node ./scripts/smoke-web.mjs`
6. Then set `VITE_API_URL=https://api.banal.app` and redeploy Production (only after Railway custom domain is also Valid — see [cookie-cutover.md](cookie-cutover.md)).
...
| Production  | `banal-web-production` | Planned: `https://app.banal.app` (interim `*.vercel.app`) | 2026-07-25 |
